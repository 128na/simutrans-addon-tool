...
export default function registerAutoPakApi(mainWindow: BrowserWindow): void {
  const errorHandler = (error: unknown) => {
    if (error instanceof Error) {
      console.log('[errorHandler] type is ', error?.name);
      if (error.name === 'AbortError') {
        return mainWindow.webContents.send('updatePak', 'error', '処理を中断しました');
      }
      return mainWindow.webContents.send('updatePak', 'error', error.message);
    }
    console.log('[errorHandler] unknown error', error);
  ...
  ipcMain.on('startPak', async (event, { makeobjPath, size, pakPath, sourcePath }) => {
    try {
      ...
      const result = await builder.pak(makeobjPath, size, pakPath, sourcePath);
  ...
  ipcMain.on('startAutoPak', async (event, { makeobjPath, size, pakPath, simutransPath, sourcePath }) => {
    ...
    const onReady = (pathes: onReadyArgs) => {
      console.log('[onReady]');
      mainWindow.webContents.send('updateAutoPak', 'debug', '監視準備完了', pathes);
      doProcess();
    };
    ...
    const doProcess = async () => {
      ...
      if (running) {
        ...
        mainWindow.webContents.send('updateAutoPak', 'info', '前のpak化完了待ち');
        return;
      }
      ...
      try {
        running = true;
        await builder.stop();
        mainWindow.webContents.send('updateAutoPak', 'debug', 'Pakファイル作成開始');
        ...
        if (result.status === 0) {
          ...
          mainWindow.webContents.send('updateAutoPak', 'success', result.stdout);
          simutrans.run();
          mainWindow.webContents.send('updateAutoPak', 'success', 'Simutrans起動中');
        } else {
          mainWindow.webContents.send('updateAutoPak', 'error', result.stderr);
        }
      } catch (error: unknown) {
        errorHandler(error);
      } finally {
