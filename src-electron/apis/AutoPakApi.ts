  ...
  ipcMain.handle('startAutoPak', async (event, { makeobjPath, size, pakPath, simutransPath, sourcePath }) => {
    ...
    const doProcess = async () => {
      ...
      if (running) {
        ...
        mainWindow.webContents.send('[doProcess] updateAutoPak', 'info', '前のpak化完了待ち');
        return;
      }
      ...
      try {
        ...
        if (error instanceof Error) {
          mainWindow.webContents.send('updateAutoPak', 'error', error.message);
        } else {
          mainWindow.webContents.send('updateAutoPak', 'エラーが発生しました', error);
        }
      } finally {
